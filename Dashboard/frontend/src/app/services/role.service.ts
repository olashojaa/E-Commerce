import { Injectable } from '@angular/core';
import { HttpClient,HttpHeaders  } from '@angular/common/http';
import { Observable } from 'rxjs';

@Injectable({
  providedIn: 'root'
})
export class RoleService {
  private apiUrl = 'http://localhost:8080/api/'; // Replace with your API endpoint

  constructor(private http: HttpClient) { }
  private getHeaders(): HttpHeaders {
    const token = localStorage.getItem('token');
    return new HttpHeaders().set('Authorization', `Bearer ${token}`);
  }

  getRoles(): Observable<any[]> {
    const headers = this.getHeaders();
    return this.http.get<any[]>(`${this.apiUrl}/roles`,{headers});
  }
  getRolebyId(roleId: number): Observable<any[]> {
    const headers = this.getHeaders();
    return this.http.get<any[]>(`${this.apiUrl}/roles/${roleId}`,{headers});
  }
  // Example POST request
  createRole(roleData: any): Observable<any> {
    const headers = this.getHeaders();
headers.set('Content-Type','application/json');
  
    return this.http.post(`${this.apiUrl}roles`, roleData,{headers});
  }
  updateRole(roleId: number, updatedRoles: any): Observable<any> {
    const headers = this.getHeaders();
    const url = `${this.apiUrl}/roles/${roleId}`;
    return this.http.put<any>(url, updatedRoles,{headers});
  }
  deleteRole(roleId: number): Observable<any> {
    const headers = this.getHeaders();
    return this.http.delete(`${this.apiUrl}/roles/${roleId}`,{headers});
  }
}
